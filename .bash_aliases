
alias ls='ls --color=auto'
alias ll='ls -sh'
alias grep='grep --color=auto'
alias feh='feh -F -Z'
#alias mpv='mpv --autofit=60% --fs --volume=35'
alias xreader='xreader -f'

export BDUSS="dadHVqbFJifjBUZVl6ZkdjaWFrRHJMM0FUOTQ4UjZUZlVuYlg2cW1GZThsSHBwSVFBQUFBJCQAAAAAAAAAAAEAAADxZYYMdGltZWJsaW5rAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAALwHU2m8B1NpMl"

alias x0='mpv --loop-playlist=3 --playlist=~/Videos/XRated/.xrated.ssh/playlist0.m3u8'
alias x1='mpv --loop-playlist=1 --playlist=~/Videos/XRated/.xrated.ssh/playlist1.m3u8'
alias x9='mpv --loop-playlist=1 --playlist=~/Videos/XRated/.xrated.cut/playlist.m3u8'
alias gq='mpv --volume=75 --fs=no --shuffle --playlist=/home/zzz/Music/老八张/playlist.m3u8'
alias baidu='baidupcs login --bduss=${BDUSS}'

