...
text_url    = os.environ['HOME']+"/Videos/XRated/.xrated.script/url_text_list.txt"
...
def get_tag_href(tag):
  ...
  tag_a_str = tag_a.find_all("li",class_="name")[0].string
...
def tag_div_class_listpic(tag):
  ...
  if not tag.has_attr('class'):
    return False
  return 'textlist' in tag.get('class')
...
if __name__ == "__main__":
  vod_url_str = '{url_root}/txt/list.html'.format(url_root=url_root_path)
  url_list = []
  for x in [1088,1089,1090,1091,1092,1093,1094,1095]:
    url_str = '{vod_url}?cate_id={id}&page=,{start},{end}'.format(
              vod_url=vod_url_str,id=x,start=1,end=350)
